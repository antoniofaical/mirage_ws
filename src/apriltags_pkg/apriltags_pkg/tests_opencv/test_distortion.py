...
stream.set(cv2.CAP_PROP_FRAME_WIDTH,1280) #NECESSÁRIO DEFINIR 
stream.set(cv2.CAP_PROP_FRAME_HEIGHT,720) #NECESSÁRIO DEFINIR
...
#Parâmetros para o 3D:
    ...
    # camera_matrix_new = camera_matrix = np.array([[1002.72, 0, 618.73], 
    #                         [0, 1008.33, 288.62], 
    #                         [0, 0, 1]], np.float32) 
    # dist_coeffs = np.array([[-0.105508, 0.40147, -0.024952, 0.00550041, -0.81168]], np.float32)

    camera_matrix_new = np.array([[963.38, 0, 670.202], 
                            [0, 960.79, 326.59], 
                            [0, 0, 1]], np.float32)
    ...
    #frame=cv2.undistort(frame,camera_matrix_new,dist_coeffs,None,camera_matrix_new)

    undistorted_april = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
...
#############################PRIMEIRA APRILTAG###############################
    ...
    if ids_TAG36H11 is not None:
      cv2.aruco.drawDetectedMarkers(frame, corners_TAG36H11, ids_TAG36H11)
      ...
      for i in range(len(ids_TAG36H11)):
            ...
            cv2.circle(frame, center_TAG36H11, 5, (0, 0, 255), -1)
...
#Pose estimation
            ...
            ret_TAG36H11,rvecs_TAG36H11, tvecs_TAG36H11 = cv2.solvePnP(objectPoints_TAG36H11, corners2_TAG36H11, camera_matrix_new, dist_coeffs)
##############################################
#Desenhando frame
            axis_TAG36H11 = np.float32([[-28,0,0], [0,-28,0], [0,0,-28]]).reshape(-1,3) #Tamanho do eixo que será plotado
            td_points_36H11,_=cv2.projectPoints(axis_TAG36H11, rvecs_TAG36H11, tvecs_TAG36H11, camera_matrix_new, dist_coeffs) #Projetando os pontos
            point_xf_TAG36H11=(int(td_points_36H11[0][0][0]), int(td_points_36H11[0][0][1])) #Definição do eixo x
            cv2.line(frame,center_TAG36H11, point_xf_TAG36H11, (0, 0, 255), 2) #Desenhando eixo x
            point_yf_TAG36H11=(int(td_points_36H11[1][0][0]), int(td_points_36H11[1][0][1])) #Definição do eixo y
            cv2.line(frame,center_TAG36H11, point_yf_TAG36H11, (0, 255, 0), 2) #Desenhando eixo y
            point_zf_TAG36H11=(int(td_points_36H11[2][0][0]),int(td_points_36H11[2][0][1])) #Definição do eixo z
            cv2.line(frame,center_TAG36H11, point_zf_TAG36H11, (255, 0, 0), 2) #Desenhando eixo z
##############################################
#Plotando a distância (Medidas em milímetros)

            distance_z_TAG36H11=str(tvecs_TAG36H11[2])
            cv2.putText(frame, "Distancia z:"+distance_z_TAG36H11+".36h11", (0, 15),
            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)

            distance_x_TAG36H11=str(tvecs_TAG36H11[0])
            cv2.putText(frame, "Distancia x:"+distance_x_TAG36H11+".36h11", (0, 60),
            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)

            distance_y_TAG36H11=str(tvecs_TAG36H11[1])
            cv2.putText(frame, "Distancia y:"+distance_y_TAG36H11+".36h11", (0, 100),
            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
...
#############################SEGUNDA APRILTAG###############################
    ...
    if ids_TAH16H5 is not None:
      ...
      cv2.aruco.drawDetectedMarkers(frame, corners_TAG16H5, ids_TAH16H5)
      ...
      for i in range(len(ids_TAH16H5)):
            ...
            cv2.circle(frame, center_TAG16H5, 5, (0, 0, 255), -1)

################################################################
#Pose estimation

            objectPoints_TAG16H5 = np.array([
                [-35, -35, 0],  # Canto superior esquerdo (em mm)
                [ 35, -35, 0],  # Canto superior direito
                [ 35,  35, 0],  # Canto inferior direito
                [-35,  35, 0]   # Canto inferior esquerdo
            ], dtype=np.float32)

            corners2_TAG16H5=corners_TAG16H5[i][0]

            ret_TAG16H5,rvecs_TAG16H5, tvecs_TAG16H5 = cv2.solvePnP(objectPoints_TAG16H5, corners2_TAG16H5, camera_matrix_new, dist_coeffs)
##############################################
#Desenhando frame

            axis_TAG16H5 = np.float32([[-28,0,0], [0,-28,0], [0,0,-28]]).reshape(-1,3) #Tamanho do eixo que será plotado
            td_points_16H5,_=cv2.projectPoints(axis_TAG16H5, rvecs_TAG16H5, tvecs_TAG16H5, camera_matrix_new, dist_coeffs) #Projetando os pontos
            point_xf_TAG16H5=(int(td_points_16H5[0][0][0]), int(td_points_16H5[0][0][1])) #Definição do eixo x
            cv2.line(frame,center_TAG16H5, point_xf_TAG16H5, (0, 0, 255), 2) #Desenhando eixo x
            point_yf_TAG16H5=(int(td_points_16H5[1][0][0]), int(td_points_16H5[1][0][1])) #Definição do eixo y
            cv2.line(frame,center_TAG16H5, point_yf_TAG16H5, (0, 255, 0), 2) #Desenhando eixo y
            point_zf_TAG16H5=(int(td_points_16H5[2][0][0]),int(td_points_16H5[2][0][1])) #Definição do eixo z
            cv2.line(frame,center_TAG16H5, point_zf_TAG16H5, (255, 0, 0), 2) #Desenhando eixo z



            distance_z_TAG16H5=str(tvecs_TAG16H5[2])
            cv2.putText(frame, distance_z_TAG16H5, (0, 30),
            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)

            distance_x_TAG16H5=str(tvecs_TAG16H5[0])
            cv2.putText(frame, distance_x_TAG16H5, (0, 75),
            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)

            distance_y_TAG16H5=str(tvecs_TAG16H5[1])
            cv2.putText(frame, distance_y_TAG16H5, (0, 115),
            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
    # Naming a window 
