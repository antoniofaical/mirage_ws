...
class TransformNode(Node):
    ...
    def transform_point(self):
        #PONTO SUGERIDO PARA TESTE:
        #x=-0.4, y=-0.4, z=-1.0
        #Point Frame
        t_point = TransformStamped()
        t_point.header.stamp = self.get_clock().now().to_msg()
        t_point.header.frame_id = 'apriltag'
        t_point.child_frame_id = 'point'
        t_point.transform.translation.x = float(self.x_point) 
        t_point.transform.translation.y = float(self.y_point)
        t_point.transform.translation.z = float(self.z_point)
...
def main(args=None):
    ...
    trasnform_frame = TransformNode()
    #Node da transformada
    while rclpy.ok():
        if(square_publisher.controle==0.0):
            print("************Recolhendo o ponto e transformando********************")
            trasnform_frame.x_point=input("Coordenada x:")
            trasnform_frame.y_point=input("Coordenada y:")
            trasnform_frame.z_point=input("Coordenada z:")
            while (trasnform_frame.trans_controll!=3):
              rclpy.spin_once(trasnform_frame)
            trasnform_frame.trans_controll=0
            print("************Planejando trajetoria********************")
            square_publisher.x_f_f=trasnform_frame.x_trans
            square_publisher.y_f_f=trasnform_frame.y_trans
            square_publisher.z_f_f=trasnform_frame.z_trans
